...
import { NodeDef, NodeDefMap } from '../../src/templates/parser.js';
...
describe('NodeDef', () => {
    ...
    it('adds refs correctly', () => {
        ...
        expect(node.hasAttribute('data-enso-node')).toBe(true);
    });
    ...
    it('adds directives correctly', () => {
        ...
        expect(node.hasAttribute('data-enso-node')).toBe(false);
        expect(node.hasAttribute('data-enso-root')).toBe(false);
    });
    ...
    it('marks node as watched when ref, event, attribute, or content is added', () => {
        ...
        expect(node2.hasAttribute('data-enso-node')).toBe(false);
        def2.addEvent('click', () => {});
        expect(node2.hasAttribute('data-enso-node')).toBe(true);

        const node3 = document.createElement('div');
        const def3 = new NodeDef('testnode3', node3, map);
        expect(node3.hasAttribute('data-enso-node')).toBe(false);
        def3.addAttribute('class', ()=>{}, new Set(['bind1']));
        expect(node3.hasAttribute('data-enso-node')).toBe(true);

        const node4 = document.createElement('div');
        const def4 = new NodeDef('testnode4', node4, map);
        expect(node4.hasAttribute('data-enso-node')).toBe(false);
        def4.addContent(null, 1, ()=>{}, new Set(['bind1']));
        expect(node4.hasAttribute('data-enso-node')).toBe(true);
    });

    it('can mark node as root', () => {
        expect(node.hasAttribute('data-enso-root')).toBe(false);
        def.markRoot();
        expect(node.hasAttribute('data-enso-root')).toBe(true);
        expect(node.getAttribute('data-enso-root')).toBe('');
        def.markRoot(true);
        expect(node.hasAttribute('data-enso-root')).toBe(true);
        expect(node.getAttribute('data-enso-root')).toBe(def.id);
    });
    ...
    it('can mark node as watched', () => {
        ...
        def.markWatched();
        expect(node.hasAttribute('data-enso-node')).toBe(true);
        expect(node.getAttribute('data-enso-node')).toBe(def.id);
    });
    ...
    it('can replace node and retain watched status', () => {
        ...
        expect(newNode.hasAttribute('data-enso-node')).toBe(true);
        expect(newNode.getAttribute('data-enso-node')).toBe(def.id);
    });
});
